...
export const cloudinaryCloudName = env.VITE_CLOUDINARY_CLOUD_NAME ?? "";
...
export function cloudinaryContained(publicId: string, size = 640) {
  return cld
    .image(publicId)
    .resize(fit().width(size).height(size))
    .delivery(format(auto()))
    .delivery(quality(autoQuality()));
}
